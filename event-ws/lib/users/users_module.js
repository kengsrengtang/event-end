    ...
    if (!newUser.phone_number) {
        ...
        dbAPI.get_db().connect(dbAPI.get_conn_url(), function(err, client, done) {
            if (err) {
                ...
                client.query("INSERT INTO Users(data) VALUES ('" + JSON.stringify(newUser) + "')", function(err, result) {
                    done();
                    if (err) {
                        error_cb(response, err);
                    } else {
...
exports.get_users_by_phone = function(phoneNumber, response, success_cb, error_cb) {
    ...
    dbAPI.get_db().connect(dbAPI.get_conn_url(), function(err, client, done) {
        if (err) {
            ...
            client.query("SELECT * FROM Users WHERE data->>'phone_number' LIKE '" + phoneNumber + "%'", function(err, result) {
                done();
                if (err) {
                    error_cb(response, err);
                } else {
    ...
    dbAPI.get_conn().query("SELECT * FROM Users WHERE id = " + dbId, function(err, result) {
        if (err) {
            ...
            if (result.length > 0)
                ...
                error_cb(response, {
                    message: "No user having user id " + dbId
                });
        }
    });
}
...
exports.delete_user = function(phoneNumber, response, success_cb, error_cb) {
    ...
    if (phoneNumber) {
        dbAPI.get_conn().query("DELETE FROM Users WHERE data->>'phone_number' = '" + phoneNumber + "'", function(err, result) {
            if (err) {
                error_cb(response, err)
            } else {
